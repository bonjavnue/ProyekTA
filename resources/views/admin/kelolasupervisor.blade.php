            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200 text-xs uppercase text-gray-500 font-bold tracking-wider">
                        <!-- <th class="p-4">Nama Supervisor</th> -->
                        <th class="p-4">Email</th>
                        <th class="p-4">Bagian yang Ditangani</th>
                ...
                <tbody class="divide-y divide-gray-100 text-sm">
                    @foreach($supervisors as $supervisor)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <!-- <td class="p-4 font-medium text-gray-900">{{ $supervisor->name }}</td> -->
                            <td class="p-4 text-gray-600">{{ $supervisor->email }}</td>
                            <td class="p-4">
                                <div class="flex flex-wrap gap-2">
                                    @if($supervisor->bagian)
                                        <span class="px-3 py-1 bg-brand-yellow/20 text-yellow-800 text-xs font-bold rounded-lg border border-brand-yellow/30">{{ $supervisor->bagian->nama_bagian }}</span>
                                    @else
                                        <span class="px-3 py-1 bg-gray-100 text-gray-600 text-xs font-bold rounded-lg border border-gray-300">Belum ada bagian</span>
                                    @endif
                                </div>
                            </td>
                            <td class="p-4 text-right">
                                <div class="flex justify-end gap-2">
                                    <button onclick="openEditModal({{ $supervisor->id }})" class="p-2 text-gray-400 hover:text-brand-yellow transition-colors" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </button>
                                    <button onclick="openDeleteModal({{ $supervisor->id }}, '{{ $supervisor->name }}')" class="p-2 text-gray-400 hover:text-brand-red transition-colors" title="Hapus">
