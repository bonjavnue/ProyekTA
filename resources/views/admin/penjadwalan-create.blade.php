...
<div class="max-w-6xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
        <!-- Header -->
        <div class="bg-gradient-to-r from-white via-white-700 to-white px-8 py-6">
            <h1 class="text-2xl font-bold text-black">Buat Jadwal Pelatihan Baru</h1>
            <p class="text-gray-500 text-sm mt-1">Isi formulir di bawah untuk membuat jadwal pelatihan</p>
        </div>

        <form action="{{ route('penjadwalan.store') }}" method="POST" class="p-8">
            @csrf
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div>
                        ...
                        <select id="id_jenis" name="id_jenis" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue focus:border-transparent outline-none transition bg-white" required>
                            ...
                            @foreach($jenisPelatihans as $jenis)
                                <option value="{{ $jenis->id_jenis }}" {{ old('id_jenis') == $jenis->id_jenis ? 'selected' : '' }}>{{ $jenis->nama_jenis }}</option>
                            @endforeach
                        </select>
                        @error('id_jenis')
                    ...
                    <div>
                        ...
                        <input type="date" id="tanggal_pelaksanaan" name="tanggal_pelaksanaan" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue focus:border-transparent outline-none transition" required value="{{ old('tanggal_pelaksanaan') }}">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="time" id="jam_mulai" name="jam_mulai" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue focus:border-transparent outline-none transition" required value="{{ old('jam_mulai') }}">
                            @error('jam_mulai')
                                <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span>
                            @enderror
                        </div>
                        <div>
                            ...
                            <input type="time" id="jam_selesai" name="jam_selesai" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue focus:border-transparent outline-none transition" required value="{{ old('jam_selesai') }}">
                            @error('jam_selesai')
                                <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div>
                        ...
                        <input type="text" id="tempat" name="tempat" placeholder="Contoh: Aula Utama Lantai 2" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue focus:border-transparent outline-none transition" required value="{{ old('tempat') }}">
                        @error('tempat')
                            <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="catatan" name="catatan" rows="2" placeholder="Catatan tambahan untuk jadwal pelatihan ini" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue focus:border-transparent outline-none transition resize-none">{{ old('catatan') }}</textarea>
                    </div>
                </div>
                        ...
                        <div class="flex gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-blue-800 leading-relaxed">
                                    Tenggat presensi akan otomatis di-set ke <strong>H+5 dari tanggal pelaksanaan (23:59)</strong>
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="space-y-2 border border-gray-200 rounded-xl p-4 max-h-64 overflow-y-auto">
                            @forelse($bagians as $bagian)
                                <label class="flex items-center cursor-pointer hover:bg-blue-50 p-2 rounded-lg transition">
                                    <input type="checkbox" name="bagians[]" value="{{ $bagian->id_bagian }}" class="w-4 h-4 text-brand-blue rounded border-gray-300 focus:ring-2 focus:ring-brand-blue cursor-pointer">
