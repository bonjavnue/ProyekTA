...
<div class="container mx-auto max-w-2xl">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="bg-gradient-to-r from-brand-blue to-blue-800 px-6 py-4">
            <h1 class="text-2xl font-bold text-white">Buat Jadwal Pelatihan Baru</h1>
        </div>

        <form action="{{ route('penjadwalan.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="date" id="tanggal_pelaksanaan" name="tanggal_pelaksanaan" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue focus:border-transparent outline-none transition" required value="{{ old('tanggal_pelaksanaan') }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="time" id="jam_mulai" name="jam_mulai" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue focus:border-transparent outline-none transition" required value="{{ old('jam_mulai') }}">
                    @error('jam_mulai')
                        <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="time" id="jam_selesai" name="jam_selesai" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue focus:border-transparent outline-none transition" required value="{{ old('jam_selesai') }}">
                    @error('jam_selesai')
                        <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div>
                ...
                <input type="text" id="tempat" name="tempat" placeholder="Contoh: Aula Utama Lantai 2" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue focus:border-transparent outline-none transition" required value="{{ old('tempat') }}">
                @error('tempat')
                    <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span>
                @enderror
            </div>
                ...
                <div class="space-y-2 border border-gray-200 rounded-lg p-4">
                    @forelse($bagians as $bagian)
                        <label class="flex items-center cursor-pointer hover:bg-blue-50 p-2 rounded transition">
                            <input type="checkbox" name="bagians[]" value="{{ $bagian->id_bagian }}" class="w-4 h-4 text-brand-blue rounded border-gray-300 focus:ring-2 focus:ring-brand-blue">
            ...
            <div>
                ...
                <textarea id="catatan" name="catatan" rows="3" placeholder="Catatan tambahan (opsional)" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-blue focus:border-transparent outline-none transition resize-none">{{ old('catatan') }}</textarea>
            </div>
